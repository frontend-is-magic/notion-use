...
非纯文字文档必须配正文说明图：默认 3000 字/图，2000–5000 字/图为检查区间，短文至少一幅。封面和图标不计入，正文与折叠附录分开检查；见 [图文密度](references/visual-density.md)。

回复经回读确认后，给每条已处理的原人工评论添加原生 🤖 reaction，跳过已选中的本账号 reaction，避免重复点击取消。见 [reaction 工作流](references/comment-reactions.md)；缺少官方接口时自动使用已授权 UI，受用户控制优先限制，不能伪报完成。
